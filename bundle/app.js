...
app.setHandler({
    ...
    name_confirm:{
      MyNameIsIntent() {
        ...
        this.$speech.addText('Is your name').addBreak('300ms').addText(this.$app.$data.temp_name+'?');
    ...
    name_state:{
      MynameSpeak(){
          ...
          this.$speech.addText('Hey').addText(name).addBreak('20ms').addText('nice to meet you.').addBreak('20ms')
    ...
    starting_position:{
      game_starter(){
        ...
        this.$speech.addText(this.$app.$data.name,this.$app.$data.enter == 1).addBreak('20ms',this.$app.$data.enter == 1)
        .addText(' You currently have ',this.$app.$data.enter == 1).addBreak('20ms',this.$app.$data.enter == 1)
        ...
        .addBreak('300ms',pushed != 0).addText('You also have pushed bet from the last game of',pushed != 0).addBreak('300ms',pushed != 0).addText(pushed,pushed != 0)
        .addBreak('300ms',pushed != 0).addText('dollars',pushed > 1).addText('dollar',pushed == 1);
    ...
    game_state:{
      ...
      automatic_win(){
        ...
        this.$speech.addText('You bet').addBreak('100ms').addSayAsCardinal(this.$app.$data.betting).addBreak('100ms').addText('dollars',this.$app.$data.betting>1).addText('dollar',this.$app.$data.betting == 1).addBreak('300ms')
        .addText('and',this.$app.$data.pushed!=0).addBreak('300ms',this.$app.$data.pushed!=0).addSayAsCardinal(this.$app.$data.pushed,this.$app.$data.pushed!=0)
        .addText('from your previous game.',this.$app.$data.pushed!=0).addBreak('300ms',this.$app.$data.pushed!=0)
        .addText('You are dealt').addBreak('300ms').addText(this.$app.$data.player_first_card).addBreak('300ms').addText(' and ').addBreak('300ms').addText(this.$app.$data.player_second_card)
        .addBreak('300ms').addText('You have Blackjack! Congratulation, you automatically win.').addBreak('300ms')
        .addText('You now have').addBreak('300ms').addSayAsCardinal(this.$app.$data.money).addBreak('300ms').addText('dollars',this.$app.$data.money>1).addText('dollar',this.$app.$data.money == 1)
        .addBreak('300ms').addText('Say "start" to keep playing!');
      ...
      automatic_lose(){
        ...
        .addText('from your previous game.',this.$app.$data.pushed!=0).addBreak('300ms',this.$app.$data.pushed!=0)
        .addText('You are dealt').addBreak('300ms').addText(this.$app.$data.player_first_card).addBreak('300ms').addText(' and ').addBreak('300ms').addText(this.$app.$data.player_second_card)
        .addBreak('300ms').addText('for a total of ').addBreak('300ms').addSayAsCardinal(this.$app.$data.player_total).addBreak('300ms',this.$app.$data.player_has_ace == 1)
        .addText('or',this.$app.$data.player_has_ace == 1).addBreak('300ms').addSayAsCardinal(this.$app.$data.player_total_2,this.$app.$data.player_has_ace == 1)
        .addBreak('300ms').addText('I have').addBreak('300ms').addText(this.$app.$data.dealer_second_card).addBreak('300ms')
        .addText('showing.').addBreak('300ms').addText('Checking if I have Blackjack.').addBreak('300ms').addText('I have ').addBreak('300ms').addText(this.$app.$data.dealer_first_card)
        .addBreak('300ms').addText(' and ').addBreak('300ms').addText(this.$app.$data.dealer_second_card).addBreak('300ms').addText('for Blackjack. I win.').addBreak('300ms')
        .addText('You now have').addBreak('300ms').addSayAsCardinal(this.$app.$data.money).addBreak('300ms').addText('dollars',this.$app.$data.money>1).addText('dollar',this.$app.$data.money == 1)
        .addBreak('300ms').addText('Say "start" to keep playing!',this.$app.$data.money != 0);
      ...
      game_start(){
        ...
        .addText('or',this.$app.$data.player_has_ace == 1).addBreak('300ms').addSayAsCardinal(this.$app.$data.player_total_2,this.$app.$data.player_has_ace == 1)
        .addBreak('300ms').addText('I have').addBreak('300ms').addText(this.$app.$data.dealer_second_card).addBreak('300ms')
        .addText('showing.').addBreak('300ms').addText('Checking if I have Blackjack.',card_score[this.$app.$data.dealer_second_card]==1 || card_score[this.$app.$data.dealer_second_card]==10)
        .addBreak('300ms').addText('I do not have Blackjack',card_score[this.$app.$data.dealer_second_card]==1 || card_score[this.$app.$data.dealer_second_card]==10)
        .addBreak('300ms').addText('Would you like to hit, or stand?')
    ...
    player_action:{
      status(){
        this.$speech.addText('You hit.').addBreak('300ms').addText('You are dealt').addBreak('300ms').addText(this.$app.$data.player_next_card).addBreak('300ms').addText('for a total of ')
        .addBreak('300ms').addSayAsCardinal(this.$app.$data.player_total).addBreak('300ms').addText('Would you like to hit, or stand?');
      ...
      status_2(){
        ...
        this.$speech.addText('you hit.').addBreak('300ms').addText('You are dealt').addBreak('300ms').addText(this.$app.$data.player_next_card).addBreak('300ms').addText('for a total of ')
        .addBreak('300ms').addSayAsCardinal(this.$app.$data.player_total).addBreak('300ms').addText('or').addBreak('300ms')
        .addSayAsCardinal(this.$app.$data.player_total_2).addBreak('300ms').addText('Would you like to hit, or stand?');
      ...
      busted(){
        ...
        this.$speech.addText('you hit.').addBreak('300ms').addText('You are dealt').addBreak('300ms').addText(this.$app.$data.player_next_card).addBreak('300ms').addText('for a total of ')
        .addBreak('300ms').addSayAsCardinal(this.$app.$data.player_total).addBreak('300ms').addText('You lost.').addBreak('300ms')
        .addText('You now have').addBreak('300ms').addSayAsCardinal(this.$app.$data.money).addBreak('300ms').addText('dollars',this.$app.$data.money>1).addText('dollar',this.$app.$data.money == 1)
        .addBreak('300ms',this.$app.$data.money != 0).addText('Say "start" to keep playing!',this.$app.$data.money != 0);
      ...
      busted_2(){
        ...
        this.$speech.addText('You hit.').addBreak('300ms').addText('You are dealt').addBreak('300ms').addText(this.$app.$data.player_next_card).addBreak('300ms').addText('for a total of ')
        .addBreak('300ms').addSayAsCardinal(this.$app.$data.player_total).addBreak('300ms').addText('or').addBreak('300ms')
        .addSayAsCardinal(this.$app.$data.player_total_2).addBreak('300ms').addText('You lost.').addBreak('300ms')
        .addText('You now have').addBreak('300ms').addSayAsCardinal(this.$app.$data.money).addBreak('300ms').addText('dollars',this.$app.$data.money>1).addText('dollar',this.$app.$data.money == 1)
        .addBreak('300ms',this.$app.$data.money != 0).addText('Say "start" to keep playing!',this.$app.$data.money != 0);
    ...
    dealer_action:{
      show(){
        ...
        .addText('I have').addBreak('300ms').addText(this.$app.$data.dealer_second_card).addBreak('300ms')
        .addText('showing.').addBreak('300ms').addText('Another card I have is').addBreak('300ms').addText(this.$app.$data.dealer_first_card).addBreak('300ms')
        .addText('for a total of').addBreak('300ms').addSayAsCardinal(this.$app.$data.dealer_total).addBreak('300ms',this.$app.$data.dealer_has_ace == 1)
        .addText('or',this.$app.$data.dealer_has_ace == 1).addBreak('300ms',this.$app.$data.dealer_has_ace == 1).addSayAsCardinal(this.$app.$data.dealer_total_2,this.$app.$data.dealer_has_ace == 1)
        .addBreak('300ms',(this.$app.$data.dealer_total < 17 && this.$app.$data.dealer_has_ace == 0) || (this.$app.$data.dealer_has_ace == 1 && this.$app.$data.dealer_total_2<17))
          ...
          while(this.$app.$data.dealer_stand < 17){
            ...
            if (hit_2 == 'ace'){
              ...
              if (this.$app.$data.dealer_total > 11) {
                  ...
                  this.$app.$data.dealer_stand = this.$app.$data.dealer_total;
                  this.$speech.addBreak('300ms').addText('I drew').addBreak('300ms').addText('ace').addBreak('300ms')
                  .addText('for a total of').addBreak('300ms').addSayAsCardinal(this.$app.$data.dealer_stand);
                  ...
                  if(this.$app.$data.dealer_total < 12){
                    ...
                    this.$app.$data.dealer_stand = this.$app.$data.dealer_total_2;
                    this.$speech.addBreak('300ms').addText('I drew').addBreak('300ms').addText('ace').addBreak('300ms')
                    .addText('for a total of').addBreak('300ms').addSayAsCardinal(this.$app.$data.dealer_total).addBreak('300ms').addText('or').addBreak('300ms')
                    ...
                    this.$app.$data.dealer_stand = this.$app.$data.dealer_total;
                    this.$speech.addBreak('300ms').addText('I drew').addBreak('300ms').addText('ace').addBreak('300ms')
                    .addText('for a total of').addBreak('300ms').addSayAsCardinal(this.$app.$data.dealer_total);
              ...
              if(this.$app.$data.dealer_total > 21){
                ...
                this.$app.$data.money = this.$app.$data.money + this.$app.$data.betting*2;

                this.$speech.addBreak('300ms').addText('I drew').addBreak('300ms').addText(hit_2).addBreak('300ms')
                .addText('for a total of').addBreak('300ms').addSayAsCardinal(this.$app.$data.dealer_total).addText('and Busted. You win.').addBreak('300ms')
                .addText('You now have').addBreak('300ms').addSayAsCardinal(this.$app.$data.money).addBreak('300ms')
              ...
              if (this.$app.$data.dealer_total > 11){
                  ...
                  if(this.$app.$data.dealer_total > 21){
                    this.$app.$data.money = this.$app.$data.money + this.$app.$data.betting*2;
                    this.$speech.addBreak('300ms').addText('I drew').addBreak('300ms').addText(hit_2).addBreak('300ms')
                    .addText('for a total of').addBreak('300ms').addSayAsCardinal(this.$app.$data.dealer_total).addBreak('300ms')
                    .addText('and Busted. You win.').addBreak('300ms')
                    .addText('You now have').addBreak('300ms').addSayAsCardinal(this.$app.$data.money).addBreak('300ms').addText('dollars',this.$app.$data.money>1).addText('dollar',this.$app.$data.money == 1)
                    .addBreak('300ms').addText('Say "start" to keep playing!');
                    this.$app.$data.dealer_speech = this.$speech;
                ...
                if(this.$app.$data.dealer_total < 12){
                  ...
                  this.$app.$data.dealer_stand = this.$app.$data.dealer_total;
                  this.$speech.addBreak('300ms').addText('I drew').addBreak('300ms').addText(hit_2).addBreak('300ms')
                  .addText('for a total of').addBreak('300ms').addSayAsCardinal(this.$app.$data.dealer_total); //only one of them is alive
                }
            }
        }
          }
        ...
        if(this.$app.$data.dealer_stand > this.$app.$data.player_stand){
          this.$speech.addBreak('300ms').addText('I stand at')
          .addBreak('300ms').addSayAsCardinal(this.$app.$data.dealer_stand).addBreak('300ms')
          .addText('You have').addBreak('300ms').addSayAsCardinal(this.$app.$data.player_stand)
          .addBreak('300ms').addText('You lose.').addBreak('300ms')
          .addText('You now have').addBreak('300ms').addSayAsCardinal(this.$app.$data.money).addBreak('300ms').addText('dollars',this.$app.$data.money>1).addText('dollar',this.$app.$data.money == 1)
          .addBreak('300ms').addText('Say "start" to keep playing!',this.$app.$data.money != 0);
          ...
          .addBreak('300ms').addSayAsCardinal(this.$app.$data.dealer_stand).addBreak('300ms')
          .addText('You have').addBreak('300ms').addSayAsCardinal(this.$app.$data.player_stand)
          .addBreak('300ms').addText('You win!').addBreak('300ms')
          .addText('You now have').addBreak('300ms').addSayAsCardinal(this.$app.$data.money).addBreak('300ms').addText('dollars',this.$app.$data.money>1).addText('dollar',this.$app.$data.money == 1)
          .addBreak('300ms').addText('Say "start" to keep playing!');
          ...
          .addBreak('300ms').addSayAsCardinal(this.$app.$data.dealer_stand).addBreak('300ms')
          .addText('You have').addBreak('300ms').addSayAsCardinal(this.$app.$data.player_stand)
          .addBreak('300ms').addText('Tied game. Your bet has been pushed over to the next game.').addBreak('300ms')
